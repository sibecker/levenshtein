...
namespace levenshtein {
    ...
    Operation const &Calculator::Cell::operation() const {
        return operation_;
    }
    ...
    Calculator::Calculator(Transforms const &transforms, SrcString const &source, DstString const &destination) :
        ...
        for (auto i = 1u; i < cache_.rows(); ++i) {
            cache_(i, 0) = Cell{Delete{source_[i - 1]}, &*cache_(i - 1, 0)};
        }
        ...
        for (auto j = 1u; j < cache_.columns(); ++j) {
            cache_(0, j) = Cell{Insert{destination_[j - 1]}, &*cache_(0, j - 1)};
        }
    }
    ...
    Calculator::Cell const &Calculator::cell(std::size_t i, std::size_t j) {
        if (!cache_(i, j)) {
            ...
            const auto rem_src = cell(i - 1, j).score() + 1;
            ...
            const auto score = std::min(std::min(rem_src, ins_dst), change);
            if (score == ins_dst) {
                // If inserting is (joint) best, choose that
                cache_(i, j) = Cell{Insert{dst}, &cell(i, j - 1)};
            } else if (score == rem_src) {
                // Otherwise, if removeing is (joint) best, choose that
                cache_(i, j) = Cell{Delete{src}, &cell(i - 1, j)};
            } else if (change_cost) {
                ...
                cache_(i, j) = Cell{Change{src, dst}, &cell(i - 1, j - 1)};
            } else {
                cache_(i, j) = Cell{Match{src, dst}, &cell(i - 1, j - 1)};
            }
        }
